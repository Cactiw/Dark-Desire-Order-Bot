...
class TradeUnion:
    ...
    def __init__(self, id, creator_id, name, players, view_link, chat_id):
        self.id = id
    ...
    def get_union(union_id=None, creator_id=None, chat_id=None, union_name=None):
        ...
        if row is None:
            return None
        return TradeUnion(row[0], row[1], row[2], row[3], row[4], row[5])

    def update_to_database(self):
        request = "update trade_unions set creator_id = %s, name = %s, players = %s, view_link = %s, chat_id = %s" \
                  " where id = %s"
        cursor.execute(request, (self.creator_id, self.name, self.players, self.view_link, self.chat_id, self.id,))
        return 0
